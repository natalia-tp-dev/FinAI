...
const profileInfo = async (req, res) => {
    try {
        ...
        const sql = 'SELECT plan_type, has_selected_plan, is_in_trial, subscription_status FROM users WHERE email = ?'
        ...
        res.status(201).json({
            ...
            has_selected_plan: userDB.has_selected_plan,
            is_in_trial: userDB.is_in_trial,
            subscription_status: userDB.subscription_status,
            isLogged
        })
    } catch (error) {
        res.status(500).json({
